...
KeyFrameEditor::playKeyFrames(int start, int end, int step)
{
  ...
  if (m_fno.count() <= 1)
    {
      setPlayFrames(false);
      Global::statusBar()->showMessage("Need more than one key frame for playing");
      qApp->processEvents();
      return;
    }
  ...
  if (start > m_fno[m_fno.count()-1])
    {
      setPlayFrames(false);
      Global::statusBar()->showMessage(QString("Current Frame (%1) greater than the last key frame (%2)"). \
		       arg(start).arg(m_fno[m_fno.count()-1]));
      qApp->processEvents();
      return;
    }
...
KeyFrameEditor::playKeyFrames()
{
  ...
  if (m_fno.count() <= 1)
    {
      setPlayFrames(false);
      Global::statusBar()->showMessage("Need more than one Key Frames for playing");
      qApp->processEvents();
      return;
    }
...
KeyFrameEditor::keyPressEvent(QKeyEvent *event)
{
  if (event->key() == Qt::Key_Space)
      ...
      if (m_selected > -1)
	{
	  ...
	  Global::statusBar()->showMessage(QString("frame at %1 copied").arg(m_copyFno));
	}
      else
	{
	  Global::statusBar()->showMessage("No frame selected for copying");
	  return;
	}
    }
    ...
    showHelp();
      ...
      if (m_copyFno < 0)
	{
	  Global::statusBar()->showMessage("No frame selected for pasting");
	  return;
	}
		  ...
		  m_selectRegion.frame1 > m_selectRegion.frame0)
		{
		  if (pfrm < m_selectRegion.frame0 &&
		      m_fno[m_selected] >= m_selectRegion.frame0)
		    {
		      ...
		      Global::statusBar()->showMessage("Cannot move keyframe into select region. Deselect region to move keyframe into that space. Right click to deselect region");
		    }
...
KeyFrameEditor::setImage(int fno, QImage img)
{
  if (fno >= m_fImage.size())
    {
      Global::statusBar()->showMessage(QString("Cannot setImage : %1 %2").	\
		       arg(fno).arg(m_fImage.size()));
      qApp->processEvents();
      return;
    }
...
KeyFrameEditor::removeKeyFrame()
{
  ...
  if (m_selected < 0)
    {
      Global::statusBar()->showMessage("No keyframe selected for removal");
      qApp->processEvents();
      return;
    }
